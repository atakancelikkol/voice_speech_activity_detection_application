"""ten_vad_rnn — TEN VAD fed through the RNNoise (xiph) noise suppressor.

The RNNoise counterpart of ten_vad_df: same production-mirroring path (8 kHz
in-place denoise -> soxr upsample to 16 kHz -> TEN VAD in 256-sample hops),
so ten_vad / ten_vad_df / ten_vad_rnn lanes in one session are a clean A/B/C
of "no cleaner vs DeepFilter-structured vs RNNoise" ahead of the VAD.

RNNoise takes no external speech hint, so unlike ten_vad_df there is no
in_speech feedback to thread through.
"""
...
class Engine(VadEngine):
    ...
    params = [
        ...
        ParamSpec("enhance", "Enhance (RNNoise)", "bool", True,
                  help="RNNoise ön-işlemesi. Kapalıyken bu lane, 8k->16k yolunu birebir "
                       "izleyen ham bir ten_vad kopyasına döner (kontrol grubu)."),
        ParamSpec("wet", "Wet mix", "float", 0.8, 0.0, 1.0, 0.05,
                  help="RNNoise çıkışı ile ham sinyalin karışımı; 1.0 = saf RNNoise. "
                       "0.8, modelin dar-bantta konuşmayı bastırma riskini sınırlar."),
    ]
    ...
    def probe(cls) -> tuple[bool, str]:
        ...
        from server.enhance.engines.rnnoise import Engine as RnEnhancer

        ok, reason = RnEnhancer.probe()
        if not ok:
            return False, reason
        return True, ""
    ...
    def __init__(self, params: dict[str, Any] | None = None):
        ...
        if self.config["enhance"]:
            ...
            self._enhancer = RnEnhancer(SOURCE_RATE, {"wet": self.config["wet"]})
        self._make_stream()
    ...
    def process(self, frame: np.ndarray, frame_start_ms: float) -> FrameScore:
        ...
        if self._enhancer is not None:
            pcm = self._enhancer.process(pcm, False)
